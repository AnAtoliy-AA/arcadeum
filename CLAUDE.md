...
- **Modern UI design** — use clean layouts, consistent spacing, and design tokens. Tokens are CSS variables minted on `<html>` from `packages/ui/src/themeDefinitions.ts` (use `var(--primary)`, `var(--glassBg)`, `bg-[var(--success)]`, etc.); static palettes are hex literals. See `/tailwind-pro` for the token → class map. Prefer polished, contemporary aesthetics over generic defaults.
