# Exploding Cats Card Implementation Plan

This document tracks the implementation status and plan for all Exploding Kittens cards.
...
Add to `ExplodingCatsState`:

```typescript
expansions: string[];  // ['attack', 'future', 'theft', 'chaos', 'deity']
```

Add to `createInitialExplodingCatsState`:

```typescript
function createInitialExplodingCatsState(
  playerIds: string[],
...
1. **`exploding-cats-turn.utils.ts`** - Attack Pack cards
2. **`exploding-cats-future.utils.ts`** - Future Pack cards
3. **`exploding-cats-theft.utils.ts`** - Theft Pack cards
4. **`exploding-cats-chaos.utils.ts`** - Chaos Pack cards
5. **`exploding-cats-deity.utils.ts`** - Deity Pack cards
...
```bash
npm run test -- --testPathPattern=exploding-cats
```
