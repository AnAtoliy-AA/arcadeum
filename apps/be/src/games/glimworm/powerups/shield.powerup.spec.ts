import { shieldPowerup } from './shield.powerup';
...
describe('shieldPowerup', () => {
  it('registers in the global POWERUP_REGISTRY under "shield"', () => {
    expect(getPowerup('shield')).toBe(shieldPowerup);
    expect(shieldPowerup.kind).toBe('shield');
    expect(shieldPowerup.durationMs).toBe(GLIMWORM_POWERUP_DURATION_MS.shield);
  });
  ...
  it('apply sets activePowerup with correct expiry, leaves speed unchanged', () => {
    ...
    shieldPowerup.apply(worm, s, NOW);
  ...
  it('expire clears activePowerup when it is still the shield kind', () => {
    const worm = makeWorm({
      activePowerup: { kind: 'shield', expiresAt: 0 },
    });
    const s = makeSession([worm]);
    expect(typeof shieldPowerup.expire).toBe('function');
    shieldPowerup.expire!(worm, s);
    expect(worm.activePowerup).toBeNull();
  });

  it('expire does NOT clear activePowerup if it is a different kind', () => {
    const worm = makeWorm({
      activePowerup: { kind: 'speed', expiresAt: 99 },
    });
    const s = makeSession([worm]);
    shieldPowerup.expire!(worm, s);
    expect(worm.activePowerup).toEqual({ kind: 'speed', expiresAt: 99 });
  });
});
