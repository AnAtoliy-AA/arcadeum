...
const supportCoffeeUrl = (
  (process.env.EXPO_PUBLIC_SUPPORT_COFFEE_URL ??
    process.env.SUPPORT_COFFEE_URL) as string | undefined
)?.trim();
const supportIban = (
...
const iosDownloadUrl = (
  process.env.EXPO_PUBLIC_IOS_DOWNLOAD_URL as string | undefined
)?.trim();
const androidDownloadUrl = (
  process.env.EXPO_PUBLIC_ANDROID_DOWNLOAD_URL as string | undefined
)?.trim();
...
const config: ExpoConfig = {
      ...
      'expo-build-properties',
      {
        android: {
          manifestPlaceholders: {
            appAuthRedirectScheme: androidRedirectScheme,
          },
        },
      },
    ],
  ...
  extra: {
    ...
    SUPPORT_COFFEE_URL:
      supportCoffeeUrl && supportCoffeeUrl.length > 0
        ? supportCoffeeUrl
        : undefined,
    SUPPORT_IBAN:
      supportIban && supportIban.length > 0 ? supportIban : undefined,
    downloadLinks: {
      ios:
        iosDownloadUrl && iosDownloadUrl.length > 0
          ? iosDownloadUrl
          : undefined,
      android:
        androidDownloadUrl && androidDownloadUrl.length > 0
          ? androidDownloadUrl
          : undefined,
    },
    eas: {
