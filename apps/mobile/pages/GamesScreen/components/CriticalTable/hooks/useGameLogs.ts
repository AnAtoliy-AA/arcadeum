...
export function useGameLogs({
  ...
  const logs = useMemo(() => {
    const source = (snapshot?.logs ?? []) as CriticalLogEntry[];
  ...
  const formatLogMessage = (message: string) => {
    ...
    if (next.startsWith('seeTheFuture.reveal:')) {
      ...
      const translatedCards = cardKeys.map((key) => {
        ...
        if (key.startsWith('cards:')) {
          ...
          return translateCardName(cardType as CriticalCard);
        }
        return key;
      });
    ...
    if (next.startsWith('stolenCard:cards:')) {
      ...
      const translatedCard = translateCardName(cardType as CriticalCard);
