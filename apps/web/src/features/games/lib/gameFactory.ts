...
export class GameFactory {
  ...
  public async loadGame(slug: string): Promise<React.ComponentType<BaseGameProps>> {
    ...
    try {
      ...
      if (typeof GameComponent !== 'function' && typeof GameComponent !== 'object') {
        throw new Error(`Game ${slug} default export is not a valid React component`);
      }
   ...
   */
  public getLoadedGame(slug: string): React.ComponentType<BaseGameProps> | null {
    return this.loadedGames.get(slug) || null;
  }

  /**
   * Clear cached game components (useful for memory management)
   */
