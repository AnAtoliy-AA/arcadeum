...
export async function getInitialTranslations(
  ...
  const loc = locale ?? (await getServerLocale());
  const [navigationMod, pwaMod, commonMod, homeData, gamesData] =
    await Promise.all([
