...
import { ScenePaletteProvider, useScenePalette } from './ScenePaletteContext';
...
describe('ScenePaletteContext', () => {
  it('exposes the provided palette to descendants', () => {
    ...
    expect(screen.getByTestId('bg').textContent).toEqual(
      palette.sceneBgGradient,
    );
  });

  it('throws when used outside a provider', () => {
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
    expect(() => render(<Consumer />)).toThrow(/ScenePaletteProvider/);
    spy.mockRestore();
  });
});
