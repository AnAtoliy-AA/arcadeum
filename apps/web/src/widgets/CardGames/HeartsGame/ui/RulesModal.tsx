...
const SECTIONS = [
  ...
  { badge: '🔄', titleKey: 'passingTitle', bodyKey: 'passing' },
...
export const RulesModal = memo(function RulesModal({
  ...
  const rules = SECTIONS.map((section) => ({
    badge: section.badge,
    title: t(
      `games.hearts_v1.rules.${section.titleKey}` as Parameters<typeof t>[0],
    ),
    body: t(
      `games.hearts_v1.rules.${section.bodyKey}` as Parameters<typeof t>[0],
    ),
  }));
  ...
  return (
    <GameRulesModal
      open={open}
      onClose={onClose}
      title={t('games.hearts_v1.rules.title')}
      icon="♥"
      variant={variant}
      rules={rules}
    />
