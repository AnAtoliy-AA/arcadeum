...
export const TurnBadge = memo(function TurnBadge({
  ...
  if (phase === 'passing') {
    ...
    label = dirKey ? t(dirKey) : t('games.hearts_v1.game.passingPhase');
  } else if (phase === 'game_over') {
    label = t('games.hearts_v1.game.gameOver');
  } else if (myTurn) {
    label = t('games.hearts_v1.game.yourTurn');
  } else if (currentEntryId) {
    label = t('games.hearts_v1.game.playerTurn', {
      player: getPlayerName(currentEntryId),
    });
  } else {
    label = t('games.hearts_v1.game.waitingForOpponent');
  }

  return (
    <div className="flex items-center justify-center">
      <div
        data-testid="hearts-turn-badge"
        className={cx(
      ...
      >
        <span
          aria-hidden="true"
          className={
            myTurn ? 'text-[var(--accent)]' : 'text-[var(--heartColor)]'
          }
        >
          ♥
        </span>
