...
```ts
gameSocket.on('games.room.kicked', decryptHandler(handleKicked, 'games.room.kicked'));
```
...
```ts
const [kickTarget, setKickTarget] = React.useState<{ id: string; name: string } | null>(null);
