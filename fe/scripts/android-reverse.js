...
function listDevices() {
  try {
    ...
    console.error('[android:reverse] Failed to list connected Android devices.', error.message);
    return [];
  }
}
...
function pickDevice(devices) {
  ...
  if (envSerial) {
    if (devices.find(([serial]) => serial === envSerial)) {
      console.log(`[android:reverse] Using device from environment: ${envSerial}`);
      return envSerial;
    }
    console.warn(`[android:reverse] Environment device ${envSerial} not found among connected devices.`);
  }
  if (devices.length === 0) {
    return null;
  }
  ...
  console.log(`[android:reverse] Multiple devices connected (${list}). Using ${chosen}. Set ANDROID_SERIAL to override.`);
  return chosen;
}
...
if (!serial) {
  console.error('[android:reverse] No available Android devices or emulators detected.');
  process.exit(1);
}
reversePort(serial);
