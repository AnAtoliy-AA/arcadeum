...
function deriveSchemeFromClientId(id) {
  ...
  return (
    ...
    id.replace(/\.apps\.googleusercontent\.com$/, '')
  );
}
...
if (!scheme) {
  console.error(
    'Missing AUTH_IOS_REDIRECT_SCHEME (or AUTH_IOS_CLIENT_ID to derive it).',
  );
  process.exit(1);
}
...
if (/\.apps\.googleusercontent\.com$/.test(scheme)) {
  console.warn(
    '[warn] AUTH_IOS_REDIRECT_SCHEME looks incorrect. It should NOT include .apps.googleusercontent.com',
  );
}
...
if (!booted || parseInt(booted, 10) < 1) {
  console.error(
    'No booted simulator found. Boot one in Xcode or run: xcrun simctl boot "iPhone 15"',
  );
  process.exit(1);
}
...
if (!appPath || /No such file|Invalid/.test(appPath)) {
  console.error(
    `App ${bundleId} is not installed on the booted simulator. Build & run the app, then retry.`,
  );
  process.exit(1);
}
...
if (plist) {
  const urlTypesBlock = exec(
    `plutil -extract CFBundleURLTypes xml1 -o - '${plistPath}' 2>/dev/null || true`,
  );
  schemeFound = new RegExp(scheme.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).test(
    urlTypesBlock || plist,
  );
  console.log(urlTypesBlock || plist);
} else {
  console.warn(
    '[warn] Could not read Info.plist via plutil. Ensure Xcode command line tools are installed.',
  );
}
...
if (/Error|failed|does not/.test(openOut)) {
  ...
  console.log(
    '- Deep link invoked. If app foregrounds, redirect handling works.',
  );
}
