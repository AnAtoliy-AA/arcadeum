...
function openUrlOnBooted(url) {
  const res = spawnSync('xcrun', ['simctl', 'openurl', 'booted', url], {
    stdio: 'inherit',
  });
  if (res.status !== 0) {
    console.error(
      'Failed to open URL in iOS Simulator. Is a simulator booted?',
    );
...
if (!booted) {
  console.error(
    'No booted iOS Simulator found. Open Simulator first, then re-run this command.',
  );
  process.exit(1);
}
