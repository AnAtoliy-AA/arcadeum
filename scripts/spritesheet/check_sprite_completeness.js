...
const { checkVariantCompleteness } = require('./check-completeness');
...
function runAudit() {
  ...
  const results = variants.map((variant) =>
    checkVariantCompleteness(variant, cardSpriteMap),
  );
  ...
  results.forEach((report) => {
    const percentage = (
      (report.foundCount / report.totalExpected) *
      100
    ).toFixed(1);
    const status =
      report.missingCount === 0
        ? '✅ COMPLETE'
        : `❌ INCOMPLETE (${report.missingCount} missing)`;
    console.log(
      `${report.variant.padEnd(20)} | ${percentage.padStart(5)}% | ${status}`,
    );
    ...
    if (report.missingCount > 0 && report.missingCount < 15) {
      ...
      console.log(
        `   Missing: ${report.missingCount} types (too many to list)`,
      );
    }
  });
