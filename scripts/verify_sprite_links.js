...
const VARIANTS_DIR = path.join(__dirname, '../apps/web/src/widgets/CriticalGame/ui/styles/variants');
const PUBLIC_SPRITES_DIR = path.join(__dirname, '../apps/web/public/images/cards');
...
variantMap.forEach((filePath, variantName) => {
  ...
  if (!spriteUrlMatch) {
    console.error(`❌ [${variantName}] Missing getCardSpriteUrl or invalid format.`);
    errors++;
    return;
  }

  const spritePath = spriteUrlMatch[1];
  const fullSpritePath = path.join(__dirname, '../apps/web/public', spritePath);

  // 2. Check if file exists in public dir
  if (!fs.existsSync(fullSpritePath)) {
    console.error(`❌ [${variantName}] Sprite sheet not found at: ${spritePath}`);
    errors++;
    return;
  }
  ...
  if (stats.size < 10000) {
    console.warn(`⚠️ [${variantName}] Sprite sheet seems too small (${(stats.size / 1024).toFixed(1)} KB). Possible empty sheet.`);
    warnings++;
  } else {
    console.log(`✅ [${variantName}] Correctly linked to ${spritePath} (${(stats.size / 1024).toFixed(1)} KB)`);
  }
});
